...
async function startXeonBotInc() {
  ...
  XeonBotInc = makeWASocket({
    ...
    browser: ["SAMKIEL BOT", "Chrome", "1.0.0"],
    auth: {
      creds: state.creds,
  ...
  XeonBotInc.ev.on("connection.update", async (s) => {
    ...
    if (qr && !pairingCode) {
      console.log(
        chalk.yellow(
          "QR Code received, but pairing code is enabled. Skipping QR display."
        )
      );
    }
    ...
    if (connection === "close") {
      ...
      if (shouldReconnect) {
        ...
        console.log(
          chalk.red(
            "Connection closed permanently. Please check your session and restart the bot."
          )
        );
      }
    }
