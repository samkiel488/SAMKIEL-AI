...
/**
 * Owner verification supporting both number-based JIDs and LIDs.
...
async function isOwner(senderId) {
  try {
    ...
    console.log(`🔍 Checking owner: senderId=${senderId}, normalized=${normalized}, raw=${raw}, full=${full}, match=${match}`);
    ...
    if (match) {
      ...
      const logMessage = isLidMatch ? "✅ Owner recognized via LID" : "✅ Owner recognized via JID";
