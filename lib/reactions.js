...
async function addCommandReaction(sock, message, commandName = "") {
  try {
    if (!isAutoReactionEnabled || !message?.key?.id) {
      console.log(
        "[addCommandReaction] Auto-reaction disabled or invalid message:",
        { isAutoReactionEnabled, messageId: message?.key?.id }
      );
      return;
    }

    const cmdKey = (commandName || "").trim().toLowerCase();
    const emoji = commandEmojis.hasOwnProperty(cmdKey)
      ? commandEmojis[cmdKey]
      : commandEmojis.default;
    console.log(
      `[addCommandReaction] commandName: '${commandName}', cmdKey: '${cmdKey}', emoji: '${emoji}'`
    );
